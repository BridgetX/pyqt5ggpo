    ...
    for i in range(10):
        ...
        if r:
            ...
            if addr[1] != port or addr[0] != remote_host:
                if (addr[1] != 7000):
                    logging.info("remote end uses symmetric or restricted nat. Changing from %s:%d to %s:%d." % (
                        str(remote_host), port, str(addr[0]), addr[1]))
                    port = addr[1]
                    remote_host = addr[0]
...
def udp_proxy(args, q):
    ...
    if bindok >= 2:
        logging.info(
            "WARNING: Another instance of ggpofba seems to be running.")
        # l_sockfd.close()
        # sockfd.close()
        killGgpoFbaNG()
        time.sleep(2)
    ...
    try:
        ...
        logging.debug("first request from emulator at %s = %r" %
                      (emuaddr, emudata))
    except socket.timeout:
    ...
    try:
        ...
        logging.debug("first request from peer at %s = %r" %
                      (peeraddr, peerdata))
        logging.debug("peer %s , target %s" % (peeraddr, target))
        if peerdata and " _" in peerdata:
            peerdata, peeraddr = sockfd.recvfrom(16384)
            logging.debug("request from peer at %s = %r" %
                          (peeraddr, peerdata))
        ...
        if peerdata and " ok" not in peerdata and " _" not in peerdata:
            logging.debug("sending data to emulator %s = %r" %
                          (emuaddr, peerdata))
...
def registerUriHandler():
    ...
    regKeys = []
    regKeys.append(['Software\\Classes\\fightcade',
                    '', 'URL:fightcade Protocol'])
    ...
    regKeysU = []
    regKeysU.append(['Software\\Classes\\fightcade\\shell\\open\\command',
                     '', os.path.abspath(sys.argv[0])+' "%1"'])
...
if __name__ == "__main__":
    ...
    log = os.path.join(os.path.abspath(
        os.path.dirname(sys.argv[0])), "ggpofba.log")
    errorlog = os.path.join(os.path.abspath(
        os.path.dirname(sys.argv[0])), "ggpofba-errors.log")
